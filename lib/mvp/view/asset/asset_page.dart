...
class _AssetPageState extends State<AssetPage> with AutomaticKeepAliveClientMixin implements AssetView {
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              FixedSizeText(
                ...
                style:
                TextStyle(color: Color(AppColors.GREY_1), fontSize: 12.0, fontWeight: FontWeight.bold),
              )
            ],
          ),
        ),
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Padding(
            padding: const EdgeInsets.only(left: 20.0, top: 10.0, bottom: 40.0),
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    FixedSizeText(
                      formatNum(mainTokenModel.amount + mainTokenModel.delegationAmount, 6),
                      style: TextStyle(
                        color: Colors.white,
                        fontSize: 18.0,
                      ),
  ...
  void onResponseAssetData(Map<String, dynamic> response) {
    ...
    mainTokenModel = TokenItemModel(
        name: ChainParams.MAIN_TOKEN_SHORT_NAME, symbol: ChainParams.MAIN_TOKEN_DENOM);
    ...
    if (result != null && result.isNotEmpty) {
      result.forEach((f) {
        ...
        if (f['denom'] == mainTokenModel.symbol) {
          mainTokenModel.amount = double.parse(f['amount']) / ChainParams.MAIN_TOKEN_UNIT;
        }

        if (!EXCLUDE_SYMBOL.contains(f['denom'])) {
          subTokenList.forEach((subToken) {
            if (f['denom'] == subToken.symbol) {
              subToken.amount = double.parse(f['amount']) / ChainParams.MAIN_TOKEN_UNIT;
            }
          });
        }
      });
